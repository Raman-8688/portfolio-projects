...
export class SkillsComponent implements OnInit, OnDestroy {
  ...
  buildTags = [
    ...
    'Cloud-Native Apps',
    'Enterprise Pharma Systems',
    'Real-time Dashboards',
    'RESTful APIs',
    'Containerized Deployments',
    'Schema-Isolated Databases',
  ];
  ...
  private setupScrollObserver(): void {
    setTimeout(() => {
        ...
        (entries) => {
          entries.forEach((entry) => {
            if (entry.isIntersecting) {
              ...
              else if (el.dataset['type'] === 'panel' && idx !== undefined)
                this.panelsVisible[+idx] = true;
              else if (el.dataset['type'] === 'callout')
      ...
      // Observe panels
      document
        .querySelectorAll('.skills-section .category-panel')
        .forEach((el, i) => {
      ...
      if (callout) {
        (callout as HTMLElement).dataset['type'] = 'callout';
        this.observer.observe(callout);
      }
    }, 300);
  }
}
